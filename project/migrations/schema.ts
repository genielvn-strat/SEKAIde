import { pgTable, unique, uuid, text, integer, foreignKey, boolean, timestamp, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const color = pgEnum("color", ['red', 'orange', 'yellow', 'green', 'blue', 'violet'])
...
export const roles = pgTable("roles", {
	...
	unique("roles_nameId_unique").on(table.nameId),
]);
...
export const lists = pgTable("lists", {
	...
	position: integer().notNull(),
	isFinal: boolean().default(false).notNull(),
	color: color(),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow(),
}, (table) => [
